...
- PROJECT_CONTEXT.md 全文重写 v1.2 → v1.3（治理层重构第 2 步，对齐 Solo_Product_Principles v1.1）
  - §1 项目名称：项目代号 `Solo-Life-OS` 保留，对外产品名 `Solo`，定位改为"AI 驱动的日常体验发现产品"（移除"Life Operating System"对外定位）
  ...
  - §7 产品模块：Today 重构为"一次一份体验提案"（6 阶段核心对象生命周期）；Growth/Community 移除（活动归入 Explore）；Mood 收敛为"此刻状态输入"；Story 暂缓；AI Platform 6 角色定义
  - §8 产品边界：补全商业推荐边界（推荐信任不可出售）
  - §9 产品发展阶段：改为产品宪法 §十四 演进顺序（一件事→一段连接→一个下午→Day Curation），明确 Phase 不绑定 Sprint
  - §11 AI 架构原则：6 角色定义 + 置信度门控 + 主动通知边界 + AI 不扮演关系
  - §12 数据资产：新增 ExperienceOpportunity/Proposal/Decision/Occurrence/Feedback/LifeResponseMap/ExternalFact/UserStateInput；移除 Goal
  - §13 隐私边界：补全场景化数据授权 + 心理健康边界 + 推荐信任不可出售
  - §16 成功标准：北极星指标改为产品宪法 §十二 定义（每周值得体验次数），Daily Value 降为过程指标
  - §17 模块边界：MVP 模块改为 4 业务模块 + AI Platform（移除 growth/community/story）
