...
- 文档权威层级确立（治理层重构第 1 步，对齐 Solo_Product_Principles v1.1）
  ...
  - 强制阅读顺序置顶 `Solo_Product_Principles.md`（原首位 PROJECT_CONTEXT 降为第二）
  - 删除 `AGENTS §15.1` 的"文档热修直推 develop"例外条款，文档与代码同等走 feature 分支 + PR
  - `DOCUMENT_VERSION_RULE §2.1` "代码优先" 改为 "文档与代码保持同步"，涉及产品边界/领域模型/架构决策必须先 Accepted ADR
  - `DOCUMENT_VERSION_RULE §2.4` SSOT 表新增"产品宪法"为最高状态来源
